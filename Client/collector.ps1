...
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=3} -max 1000 | Get-WinEventData | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;image=$_.e_Image;dest_ip=$_.e_DestinationIP;dest_port=$_.e_DestinationPort}; Invoke-WebRequest -Uri $url/net -Method POST -Body $postParams -Headers $Headers}

# Event ID 4: Sysmon state changes (event)
$event = "Sysmon state changed"
$image = ""
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=4} | Get-WinEventData | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image=$image;details=$_.e_State}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams -Headers $Headers}

# Event ID 6: Load unsigned drivers (event)
$event = "Not signed driver loaded"
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=6} | Get-WinEventData | ? { ($_.e_SignatureStatus -notmatch '^valid.*')} | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image=$_.e_ImageLoaded;details=$_.e_Signature}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams -Headers $Headers}

# Event ID 7: Load unsigned DLLs (event) - Disabled due to high volume of false positives
# $event = "Not signed DLL loaded"
# Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=7} | Get-WinEventData | ? { ($_.e_SignatureStatus -notmatch '^valid.*')} | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image=$_.e_Image;details=$_.e_ImageLoaded}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams}
...
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=15} | Get-WinEventData | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image=$_.e_Image;details=$_.e_TargetFilename + $_.e_User}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams -Headers $Headers}

# Event ID 16: Sysmon ServiceConfigurationChange
$event = "Sysmon config changed"
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=16} | Get-WinEventData | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image=$_.UserId;details=$_.e_Configuration}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams -Headers $Headers}

# Event ID 17: PipeEvent (Pipe Created) TODO
$event = "Pipe Created"
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=17} | Get-WinEventData | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image="";details=""}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams -Headers $Headers}

# Event ID 18: PipeEvent (Pipe Connected) TODO
$event = "Pipe Connected"
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=18} | Get-WinEventData | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image="";details=""}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams -Headers $Headers}

# Event ID 19: WmiEvent (WmiEventFilter activity detected) TODO
$event = "WmiEventFilter activity"
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=19} | Get-WinEventData | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image="";details=""}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams -Headers $Headers}

# Event ID 20: WmiEvent (WmiEventConsumer activity detected) TODO
$event = "WmiEventConsumer activity"
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=20} | Get-WinEventData | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image="";details=""}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams -Headers $Headers}

# Event ID 21: WmiEvent (WmiEventConsumerToFilter activity detected) TODO
$event = "WmiEventConsumerToFilter activity"
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=21} | Get-WinEventData | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image="";details=""}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams -Headers $Headers}

# Event ID 25: ProcessTampering (Process image change) TODO
$event = "Process Tampering"
Get-WinEvent -FilterHashtable @{Logname='Microsoft-Windows-Sysmon/Operational';id=25} | Get-WinEventData | foreach{$postParams = @{date=$_.TimeCreated;host=$hostname;event=$event;image="";details=""}; Invoke-WebRequest -Uri $url/events -Method POST -Body $postParams -Headers $Headers}
